    ...
    SSL_HANDSHAKE_STATUS ssl_handshake() {
        ...
        switch (ssl_error) {
            case SSL_ERROR_WANT_READ:
                if (_last_hs != SSL_HANDSHAKE_WANT_READ) {
                    PDEBUG("[ssl] Handshake WANT_READ");
                }
                _last_hs = SSL_HANDSHAKE_WANT_READ; return SSL_HANDSHAKE_WANT_READ;
            case SSL_ERROR_WANT_WRITE:
                if (_last_hs != SSL_HANDSHAKE_WANT_WRITE) {
                    PDEBUG("[ssl] Handshake WANT_WRITE");
                }
            ...
            default:
                {
                    ...
                    if (e) {
                        ...
                        PDEBUG("[ssl] Handshake ERROR: %s", buf);
                    } else {
                        PDEBUG("[ssl] Handshake ERROR (no detail), ret=%d", ret);
                    }
