...
#include <stdint.h>
#include <stddef.h>
// Fallback: simple FNV-1a 64-bit to emulate create_sign_fs64 when ul_sign.h is unavailable
inline void create_sign_fs64(const char* data, size_t len, uint64_t* out)
{
    const uint64_t FNV_OFFSET_BASIS = 1469598103934665603ULL;
    const uint64_t FNV_PRIME = 1099511628211ULL;
    uint64_t h = FNV_OFFSET_BASIS;
    for (size_t i = 0; i < len; ++i) { h ^= (unsigned char)data[i]; h *= FNV_PRIME; }
    if (out) *out = h;
}
#endif
