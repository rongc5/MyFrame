...
static std::string gen_sec_key() {
    unsigned char rnd[16]; RAND_bytes(rnd, sizeof rnd);
    return CBase64::encode(std::string((char*)rnd, sizeof rnd));
}

static std::string calc_accept(const std::string& key) {
    static const char* GUID = "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
    std::string s = key; s += GUID;
    unsigned char sha[SHA_DIGEST_LENGTH];
    SHA1((const unsigned char*)s.data(), (unsigned long)s.size(), sha);
    return CBase64::encode(std::string((char*)sha, SHA_DIGEST_LENGTH));
}
}

class WsClient final : public IRequest {
public:
    HttpResult get(const std::string& url, const std::map<std::string,std::string>& headers) override {
        ParsedUrl u = parse_url(url);
        int fd = tcp_connect(u.host, u.port); if (fd<0) return {};
        bool use_tls = (u.scheme=="wss");
        ...
        while (true) {
            ...
            if (r<=0) break; resp.append(buf, r); if (resp.find("\r\n\r\n")!=std::string::npos) break;
        }
        ...
        // validate accept
        auto it = out.headers.find("Sec-WebSocket-Accept");
        if (out.status == 101 && it != out.headers.end()) {
            std::string expect = calc_accept(key);
            if (it->second != expect) {
                out.status = 502; // bad accept
            }
        }
...
class WsFactory final : public IRequestFactory {
public:
    std::unique_ptr<IRequest> create() override { return std::unique_ptr<IRequest>(new WsClient()); }
};

// expose helper
std::shared_ptr<IRequestFactory> make_ws_factory() { return std::shared_ptr<IRequestFactory>(new WsFactory()); }
