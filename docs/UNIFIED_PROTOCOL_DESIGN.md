...
│  - 最简单的业务处理接口                                  │
│  - 协议无关的请求/响应模型                               │
│  - 适用场景: 简单CRUD、RESTful API、简单RPC              │
└─────────────────────────────────────────────────────────┘
                         ↓ 需要协议细节时
┌─────────────────────────────────────────────────────────┐
│  Level 2: Protocol Context Handler (协议层)             │
│  - 访问协议细节和状态                                    │
│  - 支持异步、流式、状态机                               │
│  - 适用场景: WebSocket实时通信、复杂RPC、有状态协议      │
└─────────────────────────────────────────────────────────┘
                         ↓ 需要完全控制时
┌─────────────────────────────────────────────────────────┐
│  Level 3: Data Process (传输层)                         │
│  - 完全控制字节流处理                                    │
│  - 自定义协议解析和状态管理                             │
│  - 适用场景: 游戏协议、加密协议、IoT、专有二进制协议    │
└─────────────────────────────────────────────────────────┘
```
...
| 特性 | 旧设计 | 新设计 |
|------|--------|--------|
| 简单场景易用性 | ⚠️ 中等 | ✅ 优秀（Level 1） |
| 复杂场景控制力 | ⚠️ 有限 | ✅ 强大（Level 2/3） |
| 协议扩展性 | ❌ 需改框架 | ✅ 运行时注册 |
