...
class DemoHttpClientDP : public http_client_data_process {
public:
    using http_client_data_process::http_client_data_process;
    void msg_recv_finish() override {
        http_client_data_process::msg_recv_finish();
        // 优雅关闭：几乎立即触发容器统一回收（不影响其他连接/线程）
...
int main(int argc, char** argv) {
    ...
    std::map<std::string,std::string> headers; headers["User-Agent"]="http-close-demo";
    proc->set_process(new DemoHttpClientDP(proc, "GET", host, path, headers, std::string()));
